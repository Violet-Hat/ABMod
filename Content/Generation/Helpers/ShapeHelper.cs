...
namespace ABMod.Content.Generation.Helpers
{
    public class ShapeHelper
    {
        ...
        public static void PlaceOval(Point origin, int tileType, int wallType, int width, int height, double blotchChance = 0.4, bool clearTiles = true, bool clearWalls = true, bool replaceOnly = false)
        {
            //Tile placement
            ShapeData circle = new();
            GenAction blotches = new Modifiers.Blotches(2, blotchChance);

            WorldUtils.Gen(origin, new Shapes.Circle(width, height), Actions.Chain(
            [
                blotches.Output(circle)
            ]));
            ...
            WorldUtils.Gen(origin, new Shapes.Circle(width - 1, height - 1), Actions.Chain(
            [
                wallBlotches.Output(wallCircle)
            ]));
        ...
        public static void PlaceHalfCircle(Point origin, int tileType, int wallType, int radius, double blotchChance = 0.4, bool clearTiles = true, bool clearWalls = true, bool replaceOnly = false)
        {
            //Tile placement
            ...
            WorldUtils.Gen(origin, new Shapes.Circle(radius), Actions.Chain(
            [
                blotches.Output(halfCircle)
            ]));

            //Clear tiles
            if (clearTiles && !replaceOnly)
            {
                WorldUtils.Gen(origin, new ModShapes.All(halfCircle), Actions.Chain(
                [
                    new Actions.ClearTile(), new Actions.SetLiquid(0, 0)
                ]));
            }
            ...
            //Wall placement
            ShapeData wallHalfCircle = new();
            GenAction wallBlotches = new Modifiers.Blotches(2, blotchChance);

            WorldUtils.Gen(origin, new Shapes.HalfCircle(radius - 1), Actions.Chain(
            [
                wallBlotches.Output(wallHalfCircle)
            ]));

            //Clear walls
            if (clearWalls)
            {
                WorldUtils.Gen(origin, new ModShapes.All(wallHalfCircle), Actions.Chain(
                [
                    new Actions.ClearWall()
                ]));
            }
