	...
	public class Astero : ModTile
	{
		...
		public static bool Grow(int i, int j, int minSize, int maxSize, bool saplingExists = false)
		{
			...
			if((SolidTopTile(i, j + 1) || SolidTile(i, j + 1)) && !Framing.GetTileSafely(i, j).HasTile)
			{
                ...
                WorldGen.PlaceTile(i, j, ModContent.TileType<Astero>(), true);
            ...
            int transitionFrame = (int)(height / 2.5f);
			...
			for(int numSegments = 1; numSegments < height; numSegments++)
			{
				...
				if (numSegments < height - 1)
				{
                    ...
                    else if (numSegments > transitionFrame)
                    {
                        //Top frames
                        Framing.GetTileSafely(i, j - numSegments).TileFrameY = 18;
                        Framing.GetTileSafely(i, j - numSegments).TileFrameX = XFrame;
                    }
                    else //numSegments == transitionFrame
                    {
                        //Transition between trunk and top frames
		...
		public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
		{
			...
			bool up = Framing.GetTileSafely(i, j - 1).TileType == ModContent.TileType<Astero>();
            bool down = Framing.GetTileSafely(i, j + 1).TileType == ModContent.TileType<Astero>();
		...
		public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
		{
            ...
            //Draw the roots
			if((tile.TileFrameY == 0 || tile.TileFrameY == 36) && tile.TileFrameX < 54)
			{
