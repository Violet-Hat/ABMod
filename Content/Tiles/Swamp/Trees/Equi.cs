...
namespace ABMod.Content.Tiles.Swamp.Trees
{
    public class Equi : ModTile
    {
		...
		public override bool TileFrame(int i, int j, ref bool resetFrame, ref bool noBreak)
		{
            ...
            //Destroy it if there isn't a solid tile below it
            if (!tileSafely.HasTile)
            {
                WorldGen.KillTile(i, j);
            }

            //Destroy it if there isn't a valid tile below it
            if (tileSafely.TileType != ModContent.TileType<SwampSoil>() && tileSafely.TileType != ModContent.TileType<Equi>())
            {
                WorldGen.KillTile(i, j);
            }
		...
		public static bool Grow(int i, int j, int minSize, int maxSize, bool saplingExists = false)
        {
			...
			{
				//Place the bamboo segment with a random X frame
                ...
                //If it's the last segment, make it the top of the bamboo
				if(numSegments == height - 1)
                {
		...
		public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
		{
            ...
            //If there's a remaining segment below, change the frame
        ...
        //Check if the bamboo reached it's maximum size (25)
        public static bool MaxSizeReached(int x, int y)
        {
            for (int j = 0; j < 25; j++)
            {
        ...
        {
            //If the new tile is outside the world, return false
            ...
            //If the bamboo already reached it's maximum size, return false
            if (MaxSizeReached(x, y))
            {
                return false;
            }
        ...
        public override void RandomUpdate(int i, int j)
        {
            if (CanItGrow(i, j))
            {
                if (WorldGen.genRand.NextBool(10))
                {
                    ...
                    //Frame updates
                    XFrame = (short)((WorldGen.genRand.Next(5) * 18) + 270);
                    tileAbove.TileFrameX = XFrame;

                    if (tile.TileFrameX == 0)
                    {
                        XFrame = (short)((WorldGen.genRand.Next(4) * 18) + 18);
                        tile.TileFrameX = XFrame;
                    }
                    else
                    {
