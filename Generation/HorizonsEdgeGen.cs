...
namespace ABMod.Generation
{
    public class HorizonsEdgeGen : ModSystem
    {
        ...
        public static bool[] LargeAsteroidPlaced = [false, false, false]; //0 = Cave, 1 = Europa, 2 = Wide
        public static bool[] MediumAsteroidPlaced = [false, false, false]; //0 = Core, 1 = Geode, 2 = Ore
        public static bool[] SmallAsteroidPlaced = [false, false]; //0 = Pebble, 1 = Star
        ...
        private void HorizonGen(GenerationProgress progress, GameConfiguration configuration)
        {
			...
			PlaceAsteroidsY = (int)(Main.maxTilesY * 0.068f);

			//Area of the biome
			int biomeAreaX = (int)(Main.maxTilesX / 4 * 0.8f);
			int biomeAreaY = 40;
            ...
            */
            ...
            while (sizeThree > 0 && attempts++ < 10000)
            {
                ...
                int size = WorldGen.genRand.Next(30, 41);

                if (WorldgenTools.IsItPlaceable(new Point(x, y), size + 10, GenVars.structures))
                {
                    WorldUtils.Gen(new Point(x, y), new Shapes.Circle(size), Actions.Chain(new GenAction[]
                    {
            ...
            while (sizeTwo > 0 && attempts++ < 10000)
            {
                //Generate a random location
				int x = WorldGen.genRand.Next(PlaceAsteroidsX - middleArea, PlaceAsteroidsX + middleArea + 1);
				int y = WorldGen.genRand.Next(PlaceAsteroidsY - biomeAreaY, PlaceAsteroidsY + biomeAreaY + 1);

                int size = WorldGen.genRand.Next(15, 21);

                if (WorldgenTools.IsItPlaceable(new Point(x, y), size + 10, GenVars.structures))
                {
                    WorldUtils.Gen(new Point(x, y), new Shapes.Circle(size), Actions.Chain(new GenAction[]
                    {
                        new Actions.SetTile(TileID.TopazGemspark),
                    }));
            ...
            while (sizeOne > 0 && attempts++ < 10000)
            {
                //Generate a random location
        ...
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
		{
			...
			if (BiomesIndex != -1)
			{
				tasks.Insert(BiomesIndex + 1, new PassLegacy("Horizon Edge Gen", HorizonGen));
			}
		}
    }
}