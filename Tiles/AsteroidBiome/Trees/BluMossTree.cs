...
{
	public class BluMossTree : ModTile
	{
		/*
		Y = 0
		...
		*/
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(31, 93, 163), name);
		...
		public static bool Grow(int i, int j, int minSize, int maxSize, bool saplingExists = false)
		{
			...
			{
				WorldGen.PlaceTile(i, j, ModContent.TileType<BluMossTree>(), true);
			...
			for(int numSegments = 1; numSegments < height; numSegments++)
			{
				...
				WorldGen.PlaceTile(i, j - numSegments, ModContent.TileType<BluMossTree>(), true);
				...
				if(Framing.GetTileSafely(i, j - numSegments + 1).TileType != ModContent.TileType<AsteroidMossBlu>() && Framing.GetTileSafely(i, j - numSegments).TileFrameX == 0 && Framing.GetTileSafely(i, j - numSegments).TileFrameY == 0)
				{
		...
		public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
		{
			...
			bool up = Framing.GetTileSafely(i, j - 1).TileType == ModContent.TileType<BluMossTree>();
		...
		{
			//Get the textures
			TopTexture ??= ModContent.Request<Texture2D>("ABMod/Tiles/AsteroidBiome/Trees/BluMossOrb");
			LightTexture ??= ModContent.Request<Texture2D>("ABMod/Tiles/AsteroidBiome/Trees/BluMossOrb_Light");
