...
{
	public class OrgMossTree : ModTile
	{
		/*
		Y = 0
		...
		*/
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(163, 57, 31), name);
		...
		public static bool Grow(int i, int j, int minSize, int maxSize, bool saplingExists = false)
		{
			...
			{
				WorldGen.PlaceTile(i, j, ModContent.TileType<OrgMossTree>(), true);
			...
			for(int numSegments = 1; numSegments < height; numSegments++)
			{
				...
				WorldGen.PlaceTile(i, j - numSegments, ModContent.TileType<OrgMossTree>(), true);
				...
				if(Framing.GetTileSafely(i, j - numSegments + 1).TileType != ModContent.TileType<AsteroidMossOrg>() && Framing.GetTileSafely(i, j - numSegments).TileFrameX == 0 && Framing.GetTileSafely(i, j - numSegments).TileFrameY == 0)
				{
		...
		public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
		{
			...
			bool up = Framing.GetTileSafely(i, j - 1).TileType == ModContent.TileType<OrgMossTree>();
		...
		{
			//Get the textures
			TopTexture ??= ModContent.Request<Texture2D>("ABMod/Tiles/AsteroidBiome/Trees/OrgMossOrb");
			LightTexture ??= ModContent.Request<Texture2D>("ABMod/Tiles/AsteroidBiome/Trees/OrgMossOrb_Light");
