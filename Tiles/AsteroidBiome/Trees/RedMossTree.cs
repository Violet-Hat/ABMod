...
{
	public class RedMossTree : ModTile
	{
		/*
		Y = 0
		...
		*/
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(163, 31, 71), name);
		...
		public static bool Grow(int i, int j, int minSize, int maxSize, bool saplingExists = false)
		{
			...
			{
				WorldGen.PlaceTile(i, j, ModContent.TileType<RedMossTree>(), true);
			...
			for(int numSegments = 1; numSegments < height; numSegments++)
			{
				...
				WorldGen.PlaceTile(i, j - numSegments, ModContent.TileType<RedMossTree>(), true);
				...
				if(Framing.GetTileSafely(i, j - numSegments + 1).TileType != ModContent.TileType<AsteroidMossRed>() && Framing.GetTileSafely(i, j - numSegments).TileFrameX == 0 && Framing.GetTileSafely(i, j - numSegments).TileFrameY == 0)
				{
		...
		public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
		{
			...
			bool up = Framing.GetTileSafely(i, j - 1).TileType == ModContent.TileType<RedMossTree>();
		...
		{
			//Get the textures
			TopTexture ??= ModContent.Request<Texture2D>("ABMod/Tiles/AsteroidBiome/Trees/RedMossOrb");
			LightTexture ??= ModContent.Request<Texture2D>("ABMod/Tiles/AsteroidBiome/Trees/RedMossOrb_Light");
